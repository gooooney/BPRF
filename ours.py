...
import numpy as np  
from utils import Basic,rf_bayes
...
def model_train(data:list,norm=True):
    ...
    clf = get_model('Principal Component Analysis (PCA)')
    clf.fit(x_train)
    y_test_pred = clf.predict(x_test)  
    y_test_scores = clf.decision_function(x_test)  
    visualize(y_test, y_test_scores, y_test_pred,clf_name='Principal Component Analysis (PCA)')
    # visualize(y_test, y_test_scores, y_test_pred)
...
def visualize(y_test, y_test_scores, y_test_pred,clf_name = None):
    ...
    plt.suptitle(f'Model Name:{clf_name} accuracy:{asc:.2f} ')
    plt.subplot(1,2,1)
    ...
    plt.savefig(f'metrics_{clf_name}.png',dpi = 120)
    plt.show()
